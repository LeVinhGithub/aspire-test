...
import { StringFieldOptional, NumberFieldOptional, BooleanFieldOptional, DateFieldOptional, EnumFieldOptional, ObjectFieldOptional } from "src/decorators/field.decorator";
import { StatusEnum as JobApplicationStatusEnum } from "entities/job_applications";
import { StorageFile } from "entities/storage_files";

export class FilterJobOpeningRequest {
...
export class FilterJobOpeningRequestDTO {
  @ObjectFieldOptional(FilterJobOpeningRequest)
  job_openings?: FilterJobOpeningRequest;
@NumberFieldOptional({ int: true})
...
export class JobApplicationFilterJobOpeningResponse {
  id: number;
created_at: Date;
updated_at: Date;
applicant_id: number;
...
export class JobOpeningFavoriteFilterJobOpeningResponse {
  id: number;
created_at: Date;
updated_at: Date;
job_opening_id: number;
...
export class JobOpeningValuedCompetencyFilterJobOpeningResponse {
  id: number;
created_at: Date;
updated_at: Date;
job_opening_id: number;
...
export class CompanyFilterJobOpeningResponse {
  
}
export class JobTitleFilterJobOpeningResponse {
  id: number;
created_at: Date;
updated_at: Date;
name: string;
...
export class FilterJobOpeningResponse {
  id: number;
created_at: Date;
updated_at: Date;
deployment: string;
...
job_title_id: number;
job_opening_id: number;
job_applications: JobApplicationFilterJobOpeningResponse[];
job_opening_favorites: JobOpeningFavoriteFilterJobOpeningResponse[];
job_opening_valued_competencies: JobOpeningValuedCompetencyFilterJobOpeningResponse[];
...
slug: string;
image: StorageFile;
company: CompanyFilterJobOpeningResponse;
status: `${JobOpeningStatusEnum}`;
pv: number;
job_title: JobTitleFilterJobOpeningResponse;

}


export class FilterJobOpeningResponseDTO {
  job_openings: FilterJobOpeningResponse[];
    total_pages?: number;
    total_count: number;
  
  constructor(
    job_openings: (JobOpening & { job_opening_id?: number })[],
    total_count: number,
    total_pages?: number  ) {
    this.job_openings = job_openings.map((job_opening) => ({
          ...job_opening,
          id: job_opening?.id,
