...
export class GrantTokenDto {
  ...
  @ValidateIf((data: GrantTokenDto) => ["applicants","company_users"].includes(data.scope))
  phone_number?: string;

  @ValidateIf((data: GrantTokenDto) => ["applicants","company_users"].includes(data.scope))
  @StringField({ maxLength: 10, minLength: 0 })
  otp_code?: string;
  
    @StringField({"email":true,"maxLength":255,"minLength":0})
  @ValidateIf((data: GrantTokenDto) => {
    return data.grant_type === OAUTH_GRANT_TYPE.PASSWORD && ["applicants","company_users"].includes(data.scope)
  })
  email?: string;
