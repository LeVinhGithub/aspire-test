...
class AuthService
{
    ...
    public function grantToken(array $input): array
    {
        ...
        $token = match (true) {
            ...
            default => throw new \Exception('Cannot determine login method')
        };
    ...
    private function loginByEmail(array $input)
    {
        ...
        if (! $token) {
            throw new UnauthorizedHttpException('', 'Credentials is invalid');
        }
    ...
    public function verifyResetPassword(array $input, string $provider)
    {
        ...
        if (Carbon::now()->isAfter($tokenExpireTime)) {
            throw new BadRequestHttpException('Token is expired');
        }
