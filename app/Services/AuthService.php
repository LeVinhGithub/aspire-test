...
class AuthService
{
    ...
    public function applicantEmailRegistration(array $input)
    {
        $applicant = Applicant::create($input['applicant']);
    ...
    public function companyUserEmailRegistration(array $input)
    {
        $company_user = CompanyUser::create($input['company_user']);
        $token = JWTAuth::fromUser($company_user);
        event(new Registered($company_user));
