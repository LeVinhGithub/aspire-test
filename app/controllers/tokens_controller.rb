...
class TokensController < Doorkeeper::TokensController
  ...
  def resource_owner_inactive?
    return testung2s_inactive? if resource_owner.is_a?(Testung2)
  end

  def testung2s_inactive?
    return !resource_owner&.confirmed? if resource_owner.is_a?(Testung2)
  end

  # jitera-hook-for-resource-owner-locked
  def resource_owner_locked?
    return testung2s_locked? if resource_owner.is_a?(Testung2)
  end

  def testung2s_locked?
    return resource_owner&.access_locked? if resource_owner.is_a?(Testung2)
  end
end
